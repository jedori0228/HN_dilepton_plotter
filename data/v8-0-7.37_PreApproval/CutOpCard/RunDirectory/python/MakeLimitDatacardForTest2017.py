...
def MakeLimitDatacardForTest2017(logfilename, channel, mass, outputdir, outputfilename):
  ...
  out.write("bin 1\n")
  out.write("observation "+str(bkgd*LumiRatio)+"\n")
  out.write("------------\n")
  ...
  out.write("rate\t"+str(prompt*LumiRatio)+"\t"+str(fake*LumiRatio)+"\t"+str(cf*LumiRatio)+"\t"+str(signal_rate*LumiRatio)+"\n")
  out.write("------------\n")
  ...
  for a in range(firstline_index+2,len(lines)):
    ...
    for b in range(2,len(words)):
      if words[b]=="-":
        ...
        ThisSystErr = float(words[b])*0.01
        if "Stat" in line:
          ThisSystErr = ThisSystErr/math.sqrt(LumiRatio)
        syst=ThisSystErr+1.
