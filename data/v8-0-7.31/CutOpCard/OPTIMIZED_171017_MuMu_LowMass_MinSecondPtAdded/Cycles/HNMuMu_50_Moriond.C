...
void HNMuMu_50_Moriond(){

  bool DoDebug = false;

  TString CutOpCardDir = "OPTIMIZED_171017_MuMu_LowMass_MinSecondPtAdded";
  ...
  for(unsigned int i=0; i<systs.size(); i++){
    ...
    if(i==0){
      ...
      TFile *file = new TFile("pdf.root", "RECREATE");
      file->cd();
      m.pdfsyst.hist_Pdf_Replica->Write();
      m.pdfsyst.hist_Pdf_Alpha->Write();
      m.pdfsyst.hist_Pdf_Scale->Write();
      file->Close();
