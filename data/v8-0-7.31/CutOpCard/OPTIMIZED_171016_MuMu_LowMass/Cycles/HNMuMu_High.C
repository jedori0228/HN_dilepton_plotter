...
void HNMuMu_High(){
  ...
  m.filepath = WORKING_DIR+"/rootfiles/"+dataset+"/Ntuple/Skimmed_HighMass/";
  ...
  TString cutfilename = "HighMass.txt";
  ...
  for(unsigned int i=0; i<systs.size(); i++){
    ...
    if(i==0){
       cout << "Total Bkgd. = " << m.total_bkgs << endl;
       cout << "- Prompt = " << m.prompt_bkgs << " +- " << m.prompt_bkgs_syst << endl;
       cout << "- Fake = " << m.fake_bkgs << " +- " << m.fake_bkgs_syst << endl;
       cout << "- CF = " << m.cf_bkgs << " +- " << m.cf_bkgs_syst << endl;
    ...
    cout << this_syst << "\t" << y_total_bkgs.at(i) << "\t" << m.prompt_bkgs << "\t" << m.fake_bkgs << "\t" << m.cf_bkgs << endl;
  ...
  cout << endl;
  cout << "################ Sources ################" << endl;
  cout << "Source\tSystTotal\tSystPrompt\tSystFake" << endl;

  double ONLYSYST_bkg(0.), ONLYSYST_prompt(0.), ONLYSYST_fake(0.), ONLYSYST_cf(0.);
  ...
  for(unsigned int i=0; i<reldiff_means.size(); i++){
    ...
    cout << std::fixed<<std::setprecision(2) << 100.*reldiff_fake_means.at(i) << endl;
  ...
  cout << std::fixed<<std::setprecision(2) << 100.*fake_calculated_syst/y_fake_central << endl;
  ...
  cout << "Total Systematic" << "\t" << 100.*ONLYSYST_bkg << "\t" << 100.*ONLYSYST_prompt << "\t" << 100.*ONLYSYST_fake << endl;
  cout << endl;
