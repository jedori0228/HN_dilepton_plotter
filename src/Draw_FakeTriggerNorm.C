...
void Draw_FakeTriggerNorm(){
  ...
  TString plotpath = ENV_PLOT_PATH+"/"+dataset+"/FakeRateRootfiles_Norm/";
  ...
  for(unsigned int i=0; i<Leptons.size(); i++){
    ...
    if(Lepton.Contains("Muon")){
      triggers.clear();
      triggers = {"HLT_Mu3_PFJet40_v", "HLT_Mu8_TrkIsoVVL_v" ,"HLT_Mu17_TrkIsoVVL_v"};
    }
...
*/
      ...
      for(unsigned int k=0; k<vars.size(); k++){
        ...
        TString histname = trigger+"_"+var;
