...
void Draw_FakeRate_Electron(){
  ...
  float ptarray[7] = {15., 20., 30., 40., 50., 60., 70.};
  float etaarray[4] = {0., 0.8, 1.479, 2.5};
  Double_t xbins[8] = {0., 15., 20., 30., 40., 50., 60., 70.};

  TFile *outroot = new TFile(plotpath+Lepton+"_"+Sample+"_FR.root", "RECREATE");
  TH2D *histout = new TH2D(Lepton+"_"+Sample+"_FR", "", 6, ptarray, 3, etaarray);
  ...
  for(int j=0; j<3; j++){
    ...
    for(unsigned int i=0; i<triggers.size(); i++){
      ...
      TH2D *hist2d_F0 = (TH2D*)file->Get(trigger+"_Single"+Lepton+"Trigger_Dijet_Awayjet_40_events_pt_cone_vs_eta_F0");
      TH2D *hist2d_F = (TH2D*)file->Get(trigger+"_Single"+Lepton+"Trigger_Dijet_Awayjet_40_events_pt_cone_vs_eta_F");
      ...
      //==== make them {0., 15., 20., 30., 40., 50., 60., 70.};
      hist_F0 = (TH1D*)hist_F0->Rebin(7, "hnew1",xbins);
      hist_F = (TH1D*)hist_F->Rebin(7, "hnew1",xbins);
      ...
      //==== hist_F : 0., 15., 20., 30., 40., 50., 60., 70.
      //==== merge :      15., 20., 30., 40., 50., 60., 70.
      //==== i=0 (ele8)  -> 15-20
      //==== i=1 (ele12) -> 20-30
    ...
    lg->Draw();
    c1->SaveAs(plotpath+Lepton+"_"+Sample+"_Eta_bin"+TString::Itoa(j,10)+".pdf");
    c1->Close();
    delete c1;
  ...
  c_all->cd();
  lg2->Draw();
  c_all->SaveAs(plotpath+Lepton+"_"+Sample+"_alleta.pdf");
