...
void HiggsCombindedLimit_Xsec(int i=0){
  ...
  while(getline(in_SandT,elline_SandT)){
    ...
    double this_xsec = GetXsecST(mass_SandT[dummyint]);
    scale *= this_xsec;
  ...
  dummy->GetYaxis()->SetTitle("#sigma#timesBR[pp#rightarrow#mu^{#pm}N#rightarrow#mu^{#pm}#mu^{#pm}q#bar{q}] (pb)");
  ...
  dummy->Draw("hist");
  hist_axis(dummy);
  if(channel=="ElEl") dummy->GetYaxis()->SetTitle("#sigma#timesBR[pp#rightarrow#mu^{#pm}N#rightarrow#mu^{#pm}#mu^{#pm}q#bar{q}] (pb)");
  if(channel=="MuMu") dummy->GetYaxis()->SetTitle("|V_{#muN}|^{2}");
  if(channel=="MuEl") dummy->GetYaxis()->SetTitle("|V_{eN}V_{#muN}^{*}|/(|V_{eN}|^{2}+|V_{#muN}|^{2})");
  dummy->GetYaxis()->SetTitle("#sigma#timesBR[pp#rightarrow#mu^{#pm}N#rightarrow#mu^{#pm}#mu^{#pm}q#bar{q}] (pb)");
  dummy->GetXaxis()->SetTitle("m(HN) [GeV]");
...
double GetXsecST(int mass){
  ...
  if(mass==100) return 1.56E-01;
  if(mass==150) return 2.25E-02;
  if(mass==200) return 7.13E-03;
  if(mass==300) return 1.61E-03;
  if(mass==400) return 5.89E-04;
  if(mass==500) return 2.75E-04;
  if(mass==600) return 1.49E-04;
  if(mass==700) return 8.92E-05;
  if(mass==800) return 5.71E-05;
  if(mass==900) return 3.84E-05;
  if(mass==1000) return 2.68E-05;
  if(mass==1100) return 1.93E-05;
  if(mass==1200) return 1.42E-05;
  if(mass==1300) return 1.06E-05;
  if(mass==1400) return 8.05E-06;
  if(mass==1500) return 6.21E-06;
  if(mass==1700) return 3.79E-06;
  if(mass==2000) return 1.89E-06;
