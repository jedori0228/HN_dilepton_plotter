...
void fake_calculator_El_dilep(double dXYMin, double RelIsoMax, int period=0){
  ...
  TString WhichRootFile = "DiElectron";
  TString filepath = WORKING_DIR+"/rootfiles/"+dataset+"/FRCalculator_El_dxysig_DILEP/"+WhichRootFile+"/";
  TString plotpath = ENV_PLOT_PATH+"/"+dataset+"/FRCalculator_El_dxysig_DILEP/"+WhichRootFile+"/"+str_dXYCut+"/";
  ...
  all_MC_list = {"singletop", "TTJets_aMC", "VGamma", "DY", "WJets", "QCD_em", "HNEmEm_100", "VV"};
  map_string_to_MC_list["SingleElectron"] = {"singletop", "TTJets_aMC", "VGamma", "DY", "WJets"};
  map_string_to_MC_alias["SingleElectron"] = {"singletop", "t#bar{t}", "V#gamma", "DY", "W + Jets"};
  map_string_to_MC_color["SingleElectron"] = {kOrange, kRed, kSpring-7, kYellow, kGreen};
  map_string_to_MC_list["DiElectron"] = {"VGamma", "DY", "VV"};
  map_string_to_MC_alias["DiElectron"] = {"V#gamma", "DY", "VV"};
  map_string_to_MC_color["DiElectron"] = {kSpring-7, kYellow, kGreen};
  ...
  map_string_to_file["data"] = new TFile(filepath+"/FRCalculator_El_dxysig_DILEP_"+DataFileName+"DoubleEG_cat_"+catversion+".root");
  for(unsigned int i=0; i<all_MC_list.size(); i++){
    TString this_samplename = all_MC_list.at(i);
    map_string_to_file[this_samplename] = new TFile(filepath+"/FRCalculator_El_dxysig_DILEP_SK"+this_samplename+"_cat_"+catversion+".root");
  ...
  cout << "######################## Loose Electron study ########################" << endl;
  
  vector<TString> type_Loose_study = {"", "HighdXY_"};
  vector<TString> var_Loose_study = {"RelIso", "dXY", "dXYSig", "pt", "eta"};
  for(unsigned int it_type_Loose_study=0; it_type_Loose_study<type_Loose_study.size(); it_type_Loose_study++){
    ...
    TString this_MC_type = "SingleElectron";
    ...
    for(unsigned int it_var_Loose_study=0; it_var_Loose_study<var_Loose_study.size(); it_var_Loose_study++){
      ...
      TString histname = str_dXYCut+"_"+this_type_Loose_study+"LooseElectron_"+this_var_Loose_study;
      ...
      c_Loose_study->SaveAs(plotpath+"/LooseElectron_Study_"+this_type_Loose_study+this_var_Loose_study+".pdf");
      c_Loose_study->Close();
  ...
  vector<TString> ElectronId = {"Loose", "Tight"};

  for(unsigned int i=0; i<ElectronId.size(); i++){
    for(unsigned int j=0; j<dXYs.size(); j++){
      
      TH1D* hist_DY = (TH1D*)map_string_to_file["DY"]->Get(str_dXYCut+"_prompt_"+ElectronId.at(i)+"_"+dXYs.at(j));
      TH1D* hist_signal = (TH1D*)map_string_to_file["HNEmEm_100"]->Get(str_dXYCut+"_prompt_"+ElectronId.at(i)+"_"+dXYs.at(j));
      TH1D* hist_ttbar = (TH1D*)map_string_to_file["TTJets_aMC"]->Get(str_dXYCut+"_fake_"+ElectronId.at(i)+"_"+dXYs.at(j));
      TH1D* hist_QCD_em = (TH1D*)map_string_to_file["QCD_em"]->Get(str_dXYCut+"_fake_"+ElectronId.at(i)+"_"+dXYs.at(j));
      
      hist_DY->SetLineColor(kBlue);
      hist_signal->SetLineColor(kBlack);
      hist_ttbar->SetLineColor(kRed);
      hist_QCD_em->SetLineColor(kOrange-5);
      
      hist_DY->SetLineWidth(2);
      hist_signal->SetLineWidth(2);
      hist_ttbar->SetLineWidth(2);
      hist_QCD_em->SetLineWidth(2);
      ...
      c_dXY_eff->cd();
      double xbins[n_xbin];
      double eff_DY_small[n_xbin], eff_signal_small[n_xbin], eff_ttbar_small[n_xbin], eff_QCD_em_small[n_xbin];
      double eff_DY_large[n_xbin], eff_signal_large[n_xbin], eff_ttbar_large[n_xbin], eff_QCD_em_large[n_xbin];
      ...
      for(unsigned int k=1; k<=n_xbin; k++){
        ...
        eff_QCD_em_small[k-1] = hist_QCD_em->Integral(0, k) / hist_QCD_em->Integral(0, n_xbin+1);
        ...
        eff_QCD_em_large[k-1] = hist_QCD_em->Integral(k+1, n_xbin+1) / hist_QCD_em->Integral(0, n_xbin+1);
        ...
        cout << hist_DY->GetXaxis()->GetBinUpEdge(k) << "\t"
             ...
             << eff_QCD_em_small[k-1] << "\t" << eff_QCD_em_large[k-1] << "\t" << endl;
      }
      ...
      TGraph *gr_eff_QCD_em_small = new TGraph(n_xbin, xbins, eff_QCD_em_small);
      TGraph *gr_eff_QCD_em_large = new TGraph(n_xbin, xbins, eff_QCD_em_large);
      gr_eff_QCD_em_small->SetLineColor(kOrange-5);
      gr_eff_QCD_em_large->SetLineColor(kOrange-5);
      gr_eff_QCD_em_small->SetLineWidth(2);
      gr_eff_QCD_em_large->SetLineWidth(2);
      gr_eff_QCD_em_large->SetLineStyle(3);
      ...
      lg_eff_dXY->AddEntry(gr_eff_QCD_em_small, "Small, QCD (fake)", "l");
      ...
      lg_eff_dXY->AddEntry(gr_eff_QCD_em_large, "Large, QCD (fake)", "l");
      ...
      gr_eff_QCD_em_small->Draw("lsame");
      gr_eff_QCD_em_large->Draw("lsame");
      lg_eff_dXY->Draw();
      ...
      c_dXY_eff->SetLogy();
      c_dXY_eff->SaveAs(plotpath+"/dXYSig_Study_Eff_"+dXYs.at(j)+"_"+ElectronId.at(i)+"IsoElectron.pdf");
      c_dXY_eff->Close();
      delete c_dXY_eff;
      ...
      hist_QCD_em->Scale(1./hist_QCD_em->Integral());
      ...
      lg_dXY->AddEntry(hist_QCD_em, "QCD (fake)", "l");
      ...
      c_dXY->SaveAs(plotpath+"/dXYSig_Study_"+dXYs.at(j)+"_"+ElectronId.at(i)+"IsoElectron.pdf");
  ...
  vector<TString> FR_method = {
    ...
    "HighdXY_0bjet",
    "HighdXY_withbjet",
    "DiElectronTrigger_ZTag_Small",
    "SingleElectronTrigger_Dijet"
  };
  ...
  vector<TString> var_FR = {"eta", "pt", "RelIso", "dXY", "dXYSig", "pt_cone"};
  vector<TString> x_title_FR = {"#eta", "p_{T} [GeV]", "RelIso", "|d_{XY}| [cm]", "|SIP|", "p_{T}^{cone}"};
  ...
  for(unsigned int it_FR_method=0; it_FR_method<FR_method.size(); it_FR_method++){
    ...
    if(this_FR_method=="SingleElectronTrigger_Dijet") hist_prefix = "";
    ...
    vector<TString>::iterator it_MC_END;
    TString this_MC_type = "SingleElectron";
    if(this_FR_method.Contains("DiElectron")) this_MC_type = "DiElectron";
    ...
    for(unsigned int it_var_FR=0; it_var_FR<var_FR.size(); it_var_FR++){
      ...
      if(this_FR_method == "DiElectronTrigger_ZTag_Small"){
        if(this_var_FR == "pt"){
          num_MC_stack->SetMaximum(10000000);
          num_MC_stack->SetMinimum(1);
          den_MC_stack->SetMaximum(10000000);
      ...
      if(this_FR_method == "SingleElectronTrigger_Dijet"){
        if(this_var_FR == "pt"){
          num_MC_stack->SetMaximum(10000000);
          num_MC_stack->SetMinimum(1);
          den_MC_stack->SetMaximum(10000000);
    ...
    for(unsigned it_xyvars=0;it_xyvars<xyvars.size();it_xyvars++){
      ...
      delete c_subtracted;
      //==== write rootfile
      TString filename = plotpath+"/13TeV_dielectron_FR_"+this_FR_method+"_"+xyvar+".root";
      ...
      int n_pt_bins = 8;
      vector<double> vec_pt_bins = {10., 15., 20., 25., 30., 40., 45., 50., 70.};
  ...
  for(unsigned int i=0;i<FR_method.size();i++){
    if( FR_method.at(i).Contains("Dijet") || FR_method.at(i).Contains("DiElectronTrigger") ) continue;
  ...
  vector<TString> MC_sample_MCTruth = {"QCD_em", "TTJets_aMC", "DY", "WJets"}; //FIXME no bb sample in v8-0-2
  ...
  for(unsigned int it_MCTruth_njet=0; it_MCTruth_njet<MCTruth_njet.size(); it_MCTruth_njet++){
    ...
    for(unsigned it_xyvars=0;it_xyvars<xyvars.size();it_xyvars++){
      ...
      for(unsigned int it_MC_sample_MCTruth=0; it_MC_sample_MCTruth<MC_sample_MCTruth.size(); it_MC_sample_MCTruth++){
        ...
        TString filename_FR = plotpath+"/13TeV_dielectron_FR_"+this_MCTruth_njet+this_MC_sample_MCTruth+"_"+xyvar+".root";
  ...
  c_QCD_isodist->cd();
  TH1D *hist_qcdlarge_iso = (TH1D*)map_string_to_file["QCD_em"]->Get(str_dXYCut+"_MCTruth_HighdXY_alljet_RelIso_F0");
  TH1D *hist_qcdsmall_iso = (TH1D*)map_string_to_file["QCD_em"]->Get(str_dXYCut+"_MCTruth_alljet_RelIso_F0");
  ...
  //==== Write rootfile
  TFile *file_FR_QCD = new TFile(plotpath+"/13TeV_dielectron_FR_alljet_QCD_em_pt_vs_eta.root", "UPDATE");
  ...
  c_QCD_isodist->SaveAs(plotpath+"/QCD_em_RelIso_dXYSigs.pdf");
  c_QCD_isodist->Close();
  ...
  TH1D *hist_FR_ZTag_Large = (TH1D*)map_string_to_file["data"]->Get(str_dXYCut+"_DiElectronTrigger_ZTag_Large_dXYSig");
  TH1D *hist_FR_ZTag_Small = (TH1D*)map_string_to_file["data"]->Get(str_dXYCut+"_DiElectronTrigger_ZTag_Small_dXYSig");
  ...
  for(unsigned int i=0; i<map_string_to_MC_list["DiElectron"].size(); i++){
    TString this_sample = map_string_to_MC_list["DiElectron"].at(i);
    cout << this_sample << endl;
    TH1D *hist_Large_tmp = (TH1D*)map_string_to_file[this_sample]->Get(str_dXYCut+"_DiElectronTrigger_ZTag_Large_dXYSig");
    cout << str_dXYCut+"_DiElectronTrigger_ZTag_Large_dXYSig" << endl;
    TH1D *hist_Small_tmp = (TH1D*)map_string_to_file[this_sample]->Get(str_dXYCut+"_DiElectronTrigger_ZTag_Small_dXYSig");
  ...
  TFile *file_FR_TagZ = new TFile(plotpath+"/13TeV_dielectron_FR_DiElectronTrigger_TagZ.root", "RECREATE");
