...
void Draw_FakeDenNum(){
  ...
  TString plotpath = ENV_PLOT_PATH+"/"+dataset+"/FakeRateRootfiles_Distribution/";
  ...
  vector<TString> vars_temp = {
    "MT", "PFMET",
    "dXYSig", "dXY", "dZ",
    "eta", "pt", "pt_cone",
    "MVA",
  };
  vector<int> rebins_temp = {
    10, 10,
    10, 10, 10,
    1, 5, 5,
    1,
  ...
  vector<TString> vars;
  vector<int> rebins;
  for(unsigned int i=0; i<vars_temp.size(); i++){
    vars.push_back( vars_temp.at(i)+"_F0" );
    vars.push_back( vars_temp.at(i)+"_F" );

    rebins.push_back( rebins_temp.at(i) );
    rebins.push_back( rebins_temp.at(i) );
  }
  ...
  for(unsigned int i=0; i<Leptons.size(); i++){
    ...
    for(unsigned int k=0; k<vars.size(); k++){
      ...
      TString WhichPD = "SingleMuonTrigger";
      if(Lepton.Contains("Electron")) WhichPD = "SingleElectronTrigger";

      TString histname = WhichPD+"_Dijet_Awayjet_40_"+var;
